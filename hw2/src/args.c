  ...
  for (i = 0; optind < argc; ++i) {
    ...
    if ((option = getopt(argc, argv, "+he:")) != -1) {
      switch (option) {
        ...
        case '?': {
            if(optopt == 'e'){
            fprintf (stderr, "Option -%c requires a mandatory argument.\n", optopt);
            free(program_state);
            exit(1);
            }
        }
...
join_string_array(int count, char *array[])
{
  ...
  ret = &charArray[count + 1];

  //for (i = 0; i < count; ++i) {
    cur_str_len = strlen(array[0]);
    //memecpy(ret + len, array[0], cur_str_len);
    len += cur_str_len;
    //memecpy(ret + len, " ", 1);
    len += 1;
    cur_str_len = strlen(array[1]);
    //memecpy(ret + len, array[1], cur_str_len);
    len += cur_str_len;
    //memecpy(ret + len, " ", 1);
    len += 1;
  //}
